...
namespace MTC2016
{
    public class SubscribeMessageReceiver : IMessageReceiver
    {
        ...
        public async Task ReceiveAsync(Message message, CancellationToken cancellationToken)
        {
            ...
            {
                await _sender.SendMessageAsync(_settings.ConfirmSubscription, message.From, cancellationToken);
            }
            else
            {
                await _sender.SendMessageAsync(_settings.SubscriptionFailed, message.From, cancellationToken);
            }
        }
    }
}
